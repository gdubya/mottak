...
def rename():
    ...
    if ar.rename_object(objectstore=s3, bucket_name=bucket,
                        object_name=file, newname=newname):
        logging.info(f"Renamed {file} succesfully from {bucket} to {newname}")
        return(True)
    else:
        logging.error(f"Failed to rename {file} from {bucket} to {newname}")
        return(False)
